...
//        self.view add subview/elementwhatever ,a button that pushes modal tableview controller
    ...
    if (self.i == 1){
        UIImage *PortraitImage = [[UIImage alloc] initWithCGImage:((UIImage *)self.randomImages[self.i]).CGImage scale:1.0 orientation:UIImageOrientationRight];
        ((UIImageView *)self.imageViews[self.i]).image = PortraitImage;
    }
    if (self.i == 9) {
...
-(void)buttonAction{
    [self.navigationController presentViewController:[DefListTableViewController preLoadedView:self.cardDefs] animated:YES completion:nil];
...
//    THIS SHOULD JUST RESET THE LABEL TEXT WITH ANIMATION
    ...
    [UIView animateWithDuration:0.9 animations:^{
        ((UIImageView *)self.imageViews[self.i]).alpha = 0;
    }];
    self.i--;
}
